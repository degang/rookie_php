...
class RookieQuery {
	 ...
	 */
	function __construct(RookieMongo $mongo, $db, $collection) {
		$this->_dbName = $db;
	...
	function attr($nameOrAttrs, $value = null) {
		...
		foreach ($nameOrAttrs as $attr => $value) {
			if ($attr == "_id" && (!is_object($value) || !($value instanceof MongoId)) && strlen($value) == 24) {
	...
	function insert(array $attrs, $safe = false) {
		...
		if ($bool) {
			if ($attrs["_id"] instanceof MongoId) {
				RookieMongo::setLastInsertId($attrs["_id"]->__toString());
			}
			else {
				RookieMongo::setLastInsertId($attrs["_id"]);
			}
		}
		return $bool;
	}	
	...
	function insertNext(array $attrs) {
		...
		if ($response["ok"]) {
			RookieMongo::setLastInsertId($response["retval"]);
		}
    ...
    function updateInsert(array $where, array $array)
    {
        return $this->_collection->update($where, $array, array('upsert' => true, 'multiple' => true));
    }
    /**
     * 更新
     *
     ...
     * @return boolean
     */
    function update(array $where, array $array)
    {
        foreach ($where as $key => $val)
        {
            if ($key == '_id')
                $newWhere['_id'] = new MongoId($val);
            else
                $newWhere[$key] = $val;
        }

        return $this->_collection->update($newWhere, $array);
    }
