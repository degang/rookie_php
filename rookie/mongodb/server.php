...
class RookieServer 
{
	...
	public function __construct(array $config) {
        $config = $config[RookieModel::$hostIndex];
        ...
        $this->auth($this->_mongoUser, $this->_mongoPass);
	...
	public function auth($username, $password, $db = "admin") {
		...
		try {
			...
			$this->_mongo = new RookieMongo($server, $options);
	...
	public static function serverWithIndex($hostIndex) {
		...
		if (!isset(self::$_servers[$hostIndex])) {
			self::$_servers[$hostIndex] = new RookieServer($MONGO["servers"][$hostIndex]);
		}
